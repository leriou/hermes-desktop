...
export function onInstallProgress(callback: (progress: InstallProgress) => void): () => void {
...
// OAuth
export function oauthLogin(provider: string, profile?: string): Promise<Result> {
...
export function onOAuthLoginProgress(callback: (chunk: string) => void): () => void {
...
export function setEnv(key: string, value: string, profile?: string): Promise<boolean> {
  return invoke("set_env", { key, value, profile });
}
export function getConfig(key: string, profile?: string): Promise<string | null> {
  return invoke("get_config", { key, profile });
}
export function setConfig(key: string, value: string, profile?: string): Promise<boolean> {
...
// Model config
export function getModelConfig(profile?: string): Promise<{ provider: string; model: string; baseUrl: string }> {
  return invoke("get_model_config", { profile });
}
export function setModelConfig(provider: string, model: string, baseUrl: string, profile?: string): Promise<boolean> {
...
export function setConnectionConfig(mode: "local" | "remote" | "ssh", remoteUrl: string, apiKey?: string): Promise<boolean> {
  return invoke("set_connection_config", { mode, remoteUrl, apiKey });
}
export function setSshConfig(host: string, port: number, username: string, keyPath: string, remotePort: number, localPort: number): Promise<boolean> {
  return invoke("set_ssh_config", { host, port, username, keyPath, remotePort, localPort });
}
export function testRemoteConnection(url: string, apiKey?: string): Promise<boolean> {
  return invoke("test_remote_connection", { url, apiKey });
}
export function testSshConnection(host: string, port: number, username: string, keyPath: string, remotePort: number): Promise<boolean> {
  return invoke("test_ssh_connection", { host, port, username, keyPath, remotePort });
}
...
export function sendMessage(
  ...
  return invoke("send_message", { message, profile, resumeSessionId, history, attachments, contextFolder });
}
...
export function stageAttachment(sessionId: string, filename: string, base64Bytes: string): Promise<string> {
...
export function discoverProviderModels(
  ...
  return invoke("discover_provider_models", { provider, baseUrl, apiKey, profile });
}
...
export function onContextMenuCopyChat(callback: (format: "text" | "markdown") => void): () => void {
  return listenOnce("contextmenucopychat", callback);
}
export function onContextMenuSelectBubble(callback: (point: { x: number; y: number }) => void): () => void {
  return listenOnce("contextmenuselectbubble", callback);
}
export function onChatToolProgress(callback: (tool: string) => void): () => void {
  return listenOnce("chattoolprogress", callback);
}
export function onChatUsage(callback: (usage: {
  promptTokens: number;
...
// Platform
export function getPlatformEnabled(profile?: string): Promise<Record<string, boolean>> {
  return invoke("get_platform_enabled", { profile });
}
export function setPlatformEnabled(platform: string, enabled: boolean, profile?: string): Promise<boolean> {
...
export function syncSessionCache(profile?: string): Promise<Array<{
  id: string;
  title: string;
  startedAt: number;
  source: string;
...
export function updateSessionTitle(sessionId: string, title: string): Promise<void> {
...
// Profiles
export function listProfiles(): Promise<Array<{
  name: string;
  path: string;
  isDefault: boolean;
  isActive: boolean;
...
export function addMemoryEntry(content: string, profile?: string): Promise<Result> {
  return invoke("add_memory_entry", { content, profile });
}
export function updateMemoryEntry(index: number, content: string, profile?: string): Promise<Result> {
  return invoke("update_memory_entry", { index, content, profile });
}
export function removeMemoryEntry(index: number, profile?: string): Promise<boolean | Result> {
  return invoke("remove_memory_entry", { index, profile });
}
export function writeUserProfile(content: string, profile?: string): Promise<Result> {
  return invoke("write_user_profile", { content, profile });
}
export function writeMemory(content: string, profile?: string): Promise<Result> {
...
export function writeSoul(content: string, profile?: string): Promise<boolean | Result> {
...
// Tools
export function getToolsets(profile?: string): Promise<Array<{ key: string; label: string; description: string; enabled: boolean; source: string }>> {
  return invoke("get_toolsets", { profile });
}
export function setToolsetEnabled(key: string, enabled: boolean, profile?: string): Promise<boolean | Result> {
  return invoke("set_toolset_enabled", { key, enabled, profile });
}

// Skills
export function listInstalledSkills(profile?: string): Promise<Array<{ name: string; category: string; description: string; path: string }>> {
  return invoke("list_installed_skills", { profile });
}
export function listBundledSkills(profile?: string): Promise<Array<{
  name: string;
  description: string;
  category: string;
  source: string;
...
export function installSkill(identifier: string, profile?: string): Promise<Result> {
  return invoke("install_skill", { identifier, profile });
}
export function uninstallSkill(name: string, profile?: string): Promise<Result> {
  return invoke("uninstall_skill", { name, profile });
}

// Plugins
export function getPlugins(profile?: string): Promise<Array<{ name: string; description: string; enabled: boolean; version?: string; source?: string }>> {
  return invoke("get_plugins", { profile });
}
export function setPluginEnabled(name: string, enabled: boolean, profile?: string): Promise<Result> {
  return invoke("set_plugin_enabled", { name, enabled, profile });
}

// Credential Pool
export function getCredentialPool(profile?: string): Promise<Record<string, Array<{ key: string; label: string }>>> {
  return invoke("get_credential_pool", { profile });
}
export function setCredentialPool(provider: string, entries: Array<{ key: string; label: string }>, profile?: string): Promise<boolean | Result> {
  return invoke("set_credential_pool", { provider, entries, profile });
}

// Models
export function listModels(profile?: string): Promise<Array<{
  id: string;
  name: string;
  provider: string;
  model: string;
...
export function getModelAliases(profile?: string): Promise<Array<{
  name: string;
  model: string;
  provider: string;
  baseUrl: string;
...
export function addModel(
  ...
  return invoke("add_model", { name, provider, model, baseUrl, alias, profile });
}
export function removeModel(id: string, profile?: string): Promise<boolean | Result> {
  return invoke("remove_model", { id, profile });
}
export function updateModel(id: string, fields: Record<string, string>, profile?: string): Promise<boolean | Result> {
...
export function onUpdateDownloadProgress(callback: (info: { percent: number }) => void): () => void {
...
// Cron Jobs
export function listCronJobs(includeDisabled?: boolean, profile?: string): Promise<Array<{
  id: string;
  name: string;
  schedule: string;
  prompt: string;
...
export function createCronJob(schedule: string, prompt?: string, name?: string, deliver?: string, profile?: string): Promise<Result> {
  return invoke("create_cron_job", { schedule, prompt, name, deliver, profile });
}
export function updateCronJob(jobId: string, schedule?: string, prompt?: string, name?: string, deliver?: string, profile?: string): Promise<Result> {
  return invoke("update_cron_job", { jobId, schedule, prompt, name, deliver, profile });
}
export function removeCronJob(jobId: string, profile?: string): Promise<Result> {
...
export function resumeCronJob(jobId: string, profile?: string): Promise<Result> {
  return invoke("resume_cron_job", { jobId, profile });
}
export function triggerCronJob(jobId: string, profile?: string): Promise<Result> {
  return invoke("trigger_cron_job", { jobId, profile });
}
export function listCronHistory(profile?: string): Promise<Array<{
  jobId: string;
  jobName: string;
  runAt: string;
...
export function readCronOutput(path: string, profile?: string): Promise<string> {
  return invoke("read_cron_output", { path, profile });
}

// Kanban
export function kanbanListBoards(includeArchived?: boolean, profile?: string): Promise<KanbanDataResult<KanbanBoard[]> & { unsupportedMode?: boolean }> {
  return invoke("kanban_list_boards", { includeArchived, profile });
}
export function kanbanCurrentBoard(profile?: string): Promise<KanbanDataResult<string>> {
  return invoke("kanban_current_board", { profile });
}
export function kanbanSwitchBoard(slug: string, profile?: string): Promise<KanbanResult> {
  return invoke("kanban_switch_board", { slug, profile });
}
export function kanbanCreateBoard(slug: string, name?: string, switchAfter?: boolean, profile?: string): Promise<KanbanResult> {
  return invoke("kanban_create_board", { slug, name, switchAfter, profile });
}
export function kanbanRemoveBoard(slug: string, hardDelete?: boolean, profile?: string): Promise<KanbanResult> {
...
export function kanbanGetTask(taskId: string, profile?: string): Promise<KanbanDataResult<KanbanTaskDetail>> {
  return invoke("kanban_get_task", { taskId, profile });
}
export function kanbanCreateTask(input: KanbanCreateTaskInput, profile?: string): Promise<KanbanDataResult<{ id: string }>> {
...
export function kanbanAssignTask(taskId: string, assignee: string | null, profile?: string): Promise<KanbanResult> {
  return invoke("kanban_assign_task", { taskId, assignee, profile });
}
export function kanbanCompleteTask(taskId: string, result?: string, profile?: string): Promise<KanbanResult> {
  return invoke("kanban_complete_task", { taskId, result, profile });
}
export function kanbanBlockTask(taskId: string, reason?: string, profile?: string): Promise<KanbanResult> {
  return invoke("kanban_block_task", { taskId, reason, profile });
}
export function kanbanUnblockTask(taskId: string, profile?: string): Promise<KanbanResult> {
  return invoke("kanban_unblock_task", { taskId, profile });
}
export function kanbanArchiveTask(taskId: string, profile?: string): Promise<KanbanResult> {
  return invoke("kanban_archive_task", { taskId, profile });
}
export function kanbanSpecifyTask(taskId: string, profile?: string): Promise<KanbanResult> {
  return invoke("kanban_specify_task", { taskId, profile });
}
export function kanbanReclaimTask(taskId: string, reason?: string, profile?: string): Promise<KanbanResult> {
  return invoke("kanban_reclaim_task", { taskId, reason, profile });
}
export function kanbanCommentTask(taskId: string, body: string, profile?: string): Promise<KanbanResult> {
  return invoke("kanban_comment_task", { taskId, body, profile });
}
export function kanbanDispatchOnce(dryRun?: boolean, profile?: string): Promise<KanbanDataResult<unknown>> {
...
export function runHermesBackup(profile?: string): Promise<{ success: boolean; path?: string; error?: string }> {
  return invoke("run_hermes_backup", { profile });
}
export function runHermesImport(archivePath: string, profile?: string): Promise<Result> {
...
export function discoverMemoryProviders(profile?: string): Promise<Array<{
  name: string;
  description: string;
  installed: boolean;
  active: boolean;
...
export function listMcpServers(profile?: string): Promise<Array<{ name: string; type: string; enabled: boolean; detail: string }>> {
  return invoke("list_mcp_servers", { profile });
}
export function readLogs(logFile?: string, lines?: number): Promise<{ content: string; path: string }> {
...
// Config YAML editor
export function readConfigYaml(profile?: string): Promise<{ content: string; path: string }> {
  return invoke("read_config_yaml", { profile });
}
export function writeConfigYaml(content: string, profile?: string): Promise<boolean> {
...
export function tuiCommandDispatch(sessionId: string, name: string, arg?: string): Promise<any> {
  return invoke("tui_command_dispatch", { sessionId, name, arg });
}
export function tuiCompress(sessionId: string, focusTopic?: string): Promise<any> {
...
export function tuiCreateSession(model?: string): Promise<{ session_id: string }> {
...
export function tuiSubmitPrompt(sessionId: string, text: string, profile?: string): Promise<void> {
...
export function tuiToolsConfigure(name: string, enabled: boolean, sessionId?: string): Promise<any> {
  return invoke("tui_tools_configure", { name, enabled, sessionId });
}
export function tuiApprovalRespond(sessionId: string, response: string, all?: boolean): Promise<any> {
  return invoke("tui_approval_respond", { sessionId, response, all });
}
export function tuiClarifyRespond(sessionId: string, answer: string, requestId?: string): Promise<any> {
  return invoke("tui_clarify_respond", { sessionId, answer, requestId });
}
export function tuiSudoRespond(sessionId: string, password: string, requestId?: string): Promise<any> {
  return invoke("tui_sudo_respond", { sessionId, password, requestId });
}
export function tuiSecretRespond(sessionId: string, value: string, requestId?: string): Promise<any> {
  return invoke("tui_secret_respond", { sessionId, value, requestId });
}
export function tuiSessionTitle(sessionId: string): Promise<{ title: string; session_key: string }> {
...
export function tuiSessionBranch(sessionId: string, name?: string): Promise<any> {
...
export function onTuiEvent(callback: (params: { type: string; payload: any; sid?: string }) => void): () => void {
