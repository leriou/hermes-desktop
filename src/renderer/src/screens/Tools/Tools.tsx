...
function Tools({ profile }: ToolsProps): React.JSX.Element {
  ...
  async function handleToggle(
    ...
    const result = await setToolsetEnabled(key, !currentEnabled, profile);
...
function ToolCard({
  ...
  const desc =
    i18nDesc === `tools.${tool.key}.description` ? tool.description : i18nDesc;
  ...
  return (
    ...
    >
      ...
      {desc && <div className="tools-card-description">{desc}</div>}
    </div>
  );
}
  ...
  return (
    ...
    >
      <div className="tools-card-top">
        ...
        <span
          className={`tools-status-dot ${tool.enabled ? "tools-status-on" : "tools-status-off"}`}
        />
      </div>
      <div className="tools-card-label">{tool.key}</div>
      {tool.description && (
        <div className="tools-card-description">{tool.description}</div>
      )}
    </div>
  );
}
