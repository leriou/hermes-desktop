...
export function normalizeApprovalPolicy(value: unknown): ApprovalPolicy {
  ...
  const timeoutSeconds =
    Number.isFinite(input.timeoutSeconds) && Number(input.timeoutSeconds) >= 5
      ? Math.min(600, Math.floor(Number(input.timeoutSeconds)))
      : DEFAULT_APPROVAL_POLICY.timeoutSeconds;
  const historyTtlMinutes =
    Number.isFinite(input.historyTtlMinutes) &&
    Number(input.historyTtlMinutes) >= 1
...
export function saveApprovalPolicy(policy: ApprovalPolicy): void {
  setStoreItem(
    APPROVAL_POLICY_KEY,
    JSON.stringify(normalizeApprovalPolicy(policy)),
  );
}
