...
export function ApprovalHistoryPanel({ entries }: { entries: ApprovalHistoryEntry[] }): React.JSX.Element | null {
  if (entries.length === 0) return null;
  const recent = entries.slice(-5).reverse();
  return (
    <div className="chat-approval-history" aria-label="Recent approval decisions">
      <div className="chat-approval-history-title">Recent approvals</div>
      {recent.map((entry) => (
        <div key={entry.id} className={`chat-approval-history-row chat-approval-history-${entry.decision}`}>
