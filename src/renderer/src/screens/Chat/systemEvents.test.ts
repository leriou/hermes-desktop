...
describe("systemEvents", () => {
  it("creates separate model switch events", () => {
    expect(createSystemEvent("model_switch", "Model switched", "gpt-4o-mini")).toMatchObject({
      kind: "system_event",
      role: "system",
      event: "model_switch",
      tone: "success",
  ...
  it("creates separate context compression events", () => {
    expect(createSystemEvent("context_compress", "Session compressed", "12k -> 4k")).toMatchObject({
